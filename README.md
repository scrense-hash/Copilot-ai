...
Copilot AI — это OpenAI-совместимый прокси, который интеллектуально перенаправляет запросы к различным модельм через OpenRouter.
...
**Основные компоненты:**
- `copilot_ai_service.py`: Приложение FastAPI с управлением жизненным циклом
...
# Опционально: для обхода геоблокировки
export HTTPS_PROXY="http://your-proxy-server:port"

### 3. Запуск сервиса
```bash
python copilot_ai_service.py
```
...
Настройки прокси (для обхода геоблокировки):
- `HTTP_PROXY`: HTTP прокси сервер (формат: `http://proxy.example.com:8080`)
- `HTTPS_PROXY`: HTTPS прокси сервер (приоритетнее для HTTPS запросов)
